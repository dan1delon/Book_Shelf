import{a as D}from"./assets/icons-4bffa4a2.js";import{a as p}from"./assets/vendor-3d873957.js";async function U(){const{data:t}=await p.get("https://books-backend.p.goit.global/books/category-list");return t}async function w(){const{data:t}=await p.get("https://books-backend.p.goit.global/books/top-books");return t}async function S(t){const{data:o}=await p.get(`https://books-backend.p.goit.global/books/category?category=${t}`);return o}async function F(t){const{data:o}=await p.get(`https://books-backend.p.goit.global/books/${t}`);return o}const L=document.querySelector(".category-list"),k=document.querySelector(".books-list"),C=document.querySelector(".books-section-title");async function j(){try{const t=await U();if(t.length===0)throw new Error;const e=t.map(({list_name:n})=>n).sort((n,s)=>n.localeCompare(s)).map(n=>J(n)).join("");L.insertAdjacentHTML("beforeend",e)}catch(t){console.error(t)}}j();function J(t){return`<li class="category-item">
 ...
 <ul class="books-bestsellers">
        ...
        <button class="add-to-localSt" type="button">${d?"remove from the shopping list":"add to shopping list"}</button>`;c.modalContent.innerHTML=u}catch(e){alert(`Something went wrong. You caught the following error: ${e.message}.`)}}const H=document.getElementById("loading-bar"),Z=document.querySelector(".progress-bar");function f(){H.style.opacity=1}function v(){H.style.opacity=0}function _(t){Z.style.width=`${t}%`}window.addEventListener("load",()=>{f(),_(100),setTimeout(v,500)});const V=document.querySelectorAll("button");V.forEach(t=>{t.addEventListener("click",()=>{f();let o=0;const e=setInterval(()=>{o+=10,_(o),o===100&&(clearInterval(e),v())},100)})});document.body.addEventListener("click",t=>{const o=t.target;o.tagName==="A"&&o.href.startsWith("/")&&f()});document.addEventListener("pagehide",v);
