import{a as D}from"./assets/icons-9c2ccd77.js";import{a as p}from"./assets/vendor-3d873957.js";async function U(){const{data:t}=await p.get("https://books-backend.p.goit.global/books/category-list");return t}async function w(){const{data:t}=await p.get("https://books-backend.p.goit.global/books/top-books");return t}async function S(t){const{data:o}=await p.get(`https://books-backend.p.goit.global/books/category?category=${t}`);return o}async function F(t){const{data:o}=await p.get(`https://books-backend.p.goit.global/books/${t}`);return o}const L=document.querySelector(".category-list"),k=document.querySelector(".books-list"),C=document.querySelector(".books-section-title");async function j(){try{const t=await U();if(t.length===0)throw new Error;const e=t.map(({list_name:n})=>n).sort((n,s)=>n.localeCompare(s)).map(n=>J(n)).join("");L.insertAdjacentHTML("beforeend",e)}catch(t){console.error(t)}}j();function J(t){return`<li class="category-item">
